...
class BlockSparseMLP(Module):
    ...
    def load_local(self, **kwargs):
        ...
        # ROCm: the block-sparse MoE cooperative kernel is unstable on
        # gfx1151. Force the unquantized path (which uses per-expert dispatch
        # via plain matmuls) until the RDNA MoE kernel is validated.
        if self.is_quantized and hasattr(torch.version, 'hip') and torch.version.hip is not None:
            self.is_quantized = False
