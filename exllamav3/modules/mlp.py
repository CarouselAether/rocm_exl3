...
class GatedMLP(Module):
    ...
    def load_local(self, device: torch.Device, load_slice: int, **kwargs):
        ...
        # Skipped on ROCm (same reason as attn.multi_kv): mgemm cooperative
        # kernel is unstable on gfx1151 wave32.
        ...
        self.bc = None
        # BC_GatedMLP path also uses cooperative kernels — disable on ROCm.
        if not _is_rocm and self.num_slices == 1 and self.multi_gu[0] is not None and self.downs[0].inner.bc is not None:
            mgu = self.multi_gu[0]
            self.bsz1_pa_args = [
