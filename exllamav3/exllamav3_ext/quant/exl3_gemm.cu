...
#include "../util.cuh"
#ifdef USE_ROCM
#include "exl3_gemm_kernel_rdna.hip.h"
...
#ifdef USE_ROCM
    // RDNA 3.5: the cooperative exl3_gemm kernel is unreliable for size_m == 1
    // (uses ~64KB LDS, hits RDNA WGP block-per-pair limits, grid.sync stalls).
    // The RDNA-specific gemv takes a non-cooperative path.
    // Skip when capturing a graph (gemv takes a separate path).
    if (size_m == 1 && !graph)
    {
        exl3_gemv_rdna(A, B, C, suh, A_had, svh, mcg, mul1);
        return 0;
    }
#endif
