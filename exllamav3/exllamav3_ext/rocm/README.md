...
- `hgemm.cu` — `rocm/hgemm_rdna.hip`. Verbatim copy plus two split-K GEMV kernels for
  m ≤ 8, N ≤ 256 (BC_Attention's headwise gate, 1 × 3072 @ 3072 × 48 once per layer per
  token, which rocBLAS runs as one tile workgroup at ~50× off bandwidth). Capture-safe
  (no allocations), everything else stays on rocBLAS (hipBLASLt costs prefill here).
  +10% Laguna decode. `rocm_tools/hgemm_narrow_probe.py --check` measures and verifies.
