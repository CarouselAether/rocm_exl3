...
On ROCm (gfx1151 in particular) the hipBLAS Tensile kernels have been
unreliable, so fall back to PyTorch's native at::mm which dispatches through
a path that works. On CUDA, keep the cuBLAS path for performance.
*/
...
#ifdef USE_ROCM
    // PyTorch's native at::mm on ROCm routes through a path that works on
    // gfx1151 (hipBLAS Tensile kernels for this arch have been buggy).
    int size_k = a.size(-1);
    int size_m = a.numel() / size_k;
    at::Tensor a_flat = (a.dim() == 2) ? a : a.view({size_m, size_k});
    if (output_fp32)
    {
        at::Tensor r = at::mm(a_flat.to(at::kFloat), b.to(at::kFloat));
        c.copy_(r.view_as(c));
    }
    else
    {
        at::Tensor r = at::mm(a_flat, b);
        c.copy_(r.view_as(c));
    }
#else
